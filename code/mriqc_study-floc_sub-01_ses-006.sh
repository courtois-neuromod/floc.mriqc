...
#SBATCH --job-name=mriqc_study-floc_sub-01_ses-006.job
#SBATCH --output=./code/mriqc_study-floc_sub-01_ses-006.out
#SBATCH --error=./code/mriqc_study-floc_sub-01_ses-006.err
#SBATCH --time=2:00:00
...
datalad containers-run -m 'mriqc_sub-01/ses-006' -n bids-mriqc --input sourcedata/floc/sub-01/ses-006/fmap/ --input sourcedata/floc/sub-01/ses-006/func/ --output . -- -w workdir/ --participant-label 01 --session-id 006 --omp-nthreads 8 --nprocs 8 -m bold --mem_gb 32 --no-datalad-get --bids-filter-file code/bids_filters.json --no-sub sourcedata/floc ./ participant 
mriqc_exitcode=$?
