...
#SBATCH --job-name=mriqc_study-floc_sub-02_ses-005.job
#SBATCH --output=./code/mriqc_study-floc_sub-02_ses-005.out
#SBATCH --error=./code/mriqc_study-floc_sub-02_ses-005.err
#SBATCH --time=2:00:00
...
datalad containers-run -m 'mriqc_sub-02/ses-005' -n bids-mriqc --input sourcedata/floc/sub-02/ses-005/fmap/ --input sourcedata/floc/sub-02/ses-005/func/ --output . -- -w workdir/ --participant-label 02 --session-id 005 --omp-nthreads 8 --nprocs 8 -m bold --mem_gb 32 --no-datalad-get --bids-filter-file code/bids_filters.json --no-sub sourcedata/floc ./ participant 
mriqc_exitcode=$?
