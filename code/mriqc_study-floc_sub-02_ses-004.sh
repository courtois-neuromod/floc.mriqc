...
#SBATCH --job-name=mriqc_study-floc_sub-02_ses-004.job
#SBATCH --output=./code/mriqc_study-floc_sub-02_ses-004.out
#SBATCH --error=./code/mriqc_study-floc_sub-02_ses-004.err
#SBATCH --time=2:00:00
...
datalad containers-run -m 'mriqc_sub-02/ses-004' -n bids-mriqc --input sourcedata/floc/sub-02/ses-004/fmap/ --input sourcedata/floc/sub-02/ses-004/func/ --output . -- -w workdir/ --participant-label 02 --session-id 004 --omp-nthreads 8 --nprocs 8 -m bold --mem_gb 32 --no-datalad-get --bids-filter-file code/bids_filters.json --no-sub sourcedata/floc ./ participant 
mriqc_exitcode=$?
