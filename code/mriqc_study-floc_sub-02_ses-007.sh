...
#SBATCH --job-name=mriqc_study-floc_sub-02_ses-007.job
#SBATCH --output=./code/mriqc_study-floc_sub-02_ses-007.out
#SBATCH --error=./code/mriqc_study-floc_sub-02_ses-007.err
#SBATCH --time=2:00:00
...
datalad containers-run -m 'mriqc_sub-02/ses-007' -n bids-mriqc --input sourcedata/floc/sub-02/ses-007/fmap/ --input sourcedata/floc/sub-02/ses-007/func/ --output . -- -w workdir/ --participant-label 02 --session-id 007 --omp-nthreads 8 --nprocs 8 -m bold --mem_gb 32 --no-datalad-get --bids-filter-file code/bids_filters.json --no-sub sourcedata/floc ./ participant 
mriqc_exitcode=$?
...
if [ -d sourcedata/freesurfer ] ; then
    flock --verbose /lustre03/project/rrg-pbellec/ria-beluga/alias/cneuromod.floc.mriqc/.datalad_lock datalad push -J 4 -d sourcedata/freesurfer $LOCAL_DATASET --to origin
fi 
exit $mriqc_exitcode 
